...
## Usage
```
/pm:epic-decompose <project_name> <feature_name> <epic_name>
```
...
1. **Verify parameters:**
   ...
   - If not, tell user: " Parameters missing. Please run: /pm:epic-decompose <project_name> <feature_name> <epic_name>"
...
2. **Verify epic exists:**
   ...
   - Suggest: "First create it with: /pm:prd-parse $project_name $feature_name"
   - Stop execution if epic doesn't exist

3. **Check for existing tasks:**
   - Check if any numbered task files (001.md, 002.md, etc.) already exist in `./doc/$project_name/epics/$feature_name/$epic_name/`
   - If tasks exist, list them and ask: " Found {count} existing tasks. Delete and recreate all tasks? (yes/no)"
   - Only proceed with explicit 'yes' confirmation
   - If user says no, suggest: "View existing tasks with: /pm:epic-show $project_name $feature_name $epic_name"

4. **Validate epic frontmatter:**
   - Verify epic has valid frontmatter with: name, status, created, prd
   - If invalid, tell user: " Invalid epic frontmatter. Please check: ./doc/$project_name/epics/$feature_name/$epic_name.md"
...
You are decomposing an epic into specific, actionable tasks for: **$epic_name** in feature **$feature_name** (project **$project_name**)

### 1. Read the Epic
- Load the epic from `./doc/$project_name/epics/$feature_name/$epic_name.md`
...
Task:
  ...
  prompt: |
    Create task files for epic: $epic_name (feature: $feature_name, project: $project_name)

    Tasks to create:
    - {list of 3-4 tasks for this batch}

    For each task:
    1. Create file: ./doc/$project_name/epics/$feature_name/$epic_name/{number}.md
...
Save tasks as: `./doc/$project_name/epics/$feature_name/$epic_name/{task_number}.md`
...
- If dependency issues found, warn but continue: " Task dependency warning: {details}"
...
3. Next step: 
   1. "Ready to sync to GitHub? Run: /pm:epic-sync $project_name $feature_name $epic_name"
...
Aim for tasks that can be completed in 1-3 days each. Break down larger tasks into smaller, manageable pieces for the "$epic_name" epic in feature "$feature_name" (project "$project_name").